...
const GridPhaseChart: React.FC<GridPhaseChartProps> = ({ result, modelIndex, phaseKind }) => {
    ...
    const hasSelection = useRef(selectedSubstances.length > 0);

    useEffect(() => {
        if (hasSelection.current) return;
        const defaults = defaultSelectedSubstances(simulations, modelIndex, phaseKind);
        if (defaults.length === 0) return;
        hasSelection.current = true;
        setSelectedSubstances(defaults);
    }, [simulations, modelIndex, phaseKind]);
        ...
        <>
            <Autocomplete
                ...
                onOptionsChange={({ selectedItems }) => {
                    hasSelection.current = true;
                    setSelectedSubstances(selectedItems);
                }}
