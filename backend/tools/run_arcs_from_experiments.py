...
def format_simulation_request(item):
    ...
    input_keys.update(
        {key[3:]: value for key, value in species.items() if key.startswith("In_")}
    )
    ...
    cleaned_data = {
        k: (0 if math.isnan(float(v)) else float(v)) for k, v in concentrations.items()
    }
...
def convert_simulation_result_to_result(
    simulation_result: SimulationResults, scenario_id: uuid
) -> Result:
    result = Result(
        id=uuid.uuid4(),
...
def process_items():
    ...
    for item in items:
        for experiment in item["data"]["labData"]["concentrations"]["entries"]:
            ...
            result_id = db_client.upsert_result(
                convert_simulation_result_to_result(result, scenario.id)
            )
