...
def test_migration_backfills_grid_membership_up_and_down(
    ...
    with alembic_engine.connect() as conn:
        rows = conn.execute(
            text(
                "SELECT id, grid_id, grid_position FROM simulations "
                "WHERE grid_id = :grid_id ORDER BY grid_position"
            ),
            {"grid_id": grid_id},
        ).fetchall()
    ...
    with alembic_engine.connect() as conn:
        ...
        assert "grid_id" not in {c[0] for c in columns}
        assert "grid_position" not in {c[0] for c in columns}
